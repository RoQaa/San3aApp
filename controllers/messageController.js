...
exports.sendMessage = catchAsync(async (req, res, next) => {
  ...
  const sendingtime = now.toLocaleTimeString('en-US', {
    hour: 'numeric',
    minute: 'numeric',
    hour12: false,
  });
  ...
  if (deleteFrom === req.user.id || deleteTo === req.user.id) {
    ...
    if (chat.from.toString() === req.user.id && chat.rechatDateFrom !== null) {
      const messages = await Message.find({
        $and: [
          { chat: req.body.chatId },
          { date: { $gte: chat.rechatDateFrom } },
          { time: { $gte: chat.rechatTimeFrom } },
        ],
      })
        .select('-chat')
        .sort({ time: 1 })
      ...
      if (!messages) {
        return next(
          new AppError(`Sorry, can't found messages belongs to this chat `, 400)
        );
      }
      ...
      chat.from.toString() === req.user.id &&
      chat.rechatDateTo !== null
    ) {
      ...
      chat.to.toString() === req.user.id &&
      chat.rechatDateTo !== null
    ) {
      const messages = await Message.find({
        $and: [
          { chat: req.body.chatId },
          { date: { $gte: chat.rechatDateTo } },
          { time: { $gte: chat.rechatTimeTo } },
        ],
      })
        .select('-chat')
        .sort({ time: 1 })
