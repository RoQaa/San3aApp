...
const userSchema = new mongoose.Schema(
  {
    ...
    role: {
      ...
      default: 'customer',
      required: [true, 'u must choose role'],
    },
    // createdAt:{
    //     type:Date,
    //     default:Date.now(),
...
userSchema.methods.changesRateAvg = function (rate) {
  //return rate
  let sum = 0;
...
userSchema.methods.checkPaidTime = function (timeNow) {
  if (this.paidTime) {
    ...
    const paidTimeMilieSecondsAfter30Days =
      paidTime.getTime() + 30 * 24 * 60 * 60 * 1000; // => 30 days
